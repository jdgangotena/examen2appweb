...
//export function generatePDF(productoelejido: any[]) {
  ...
  const columnas = ['Descripción', 'Cantidad', 'Precio', 'Subtotal', 'IVA', 'Total'];
  const filas = productoelejido.map((producto) => [
  ...
  (doc as any).autoTable({
    head: [columnas],
    body: filas,
    startY: 30, // Ajustar la posición vertical de la tabla
    margin: { left: 14, right: 14 },
    theme: 'striped', // Estilo de tabla con rayas alternadas
    headStyles: { fillColor: [0, 0, 0], textColor: [255, 255, 255] }, // Estilo de encabezado de tabla
    bodyStyles: { valign: 'middle' }, // Alineación vertical de las celdas
    columnStyles: {
      0: { cellWidth: 60 }, // Ajusta el ancho de las columnas según sea necesario
      1: { cellWidth: 30 },
      2: { cellWidth: 30 },
      3: { cellWidth: 30 },
      4: { cellWidth: 30 },
    ...
    didDrawPage: (data: any) => {
      ...
      doc.setFontSize(10);
      doc.text('Factura generada por Sistema XYZ', 14, doc.internal.pageSize.height - 10);
    }
  });
